...
#		ET.dump(parking_event)
		...
		for element in event_root.iter("ObjectEvent"):
			...
			element.find("bizLocation").find("id").text = get_parking_gln(airport_num, parking_name)
		...
		parking_event.write("output-"+str(airport_num)+"-"+str(parking_num)+".xml", encoding="utf-8", xml_declaration=True)
